...
- **Ek bulgu (BUG #478, 14 Eyl 2026):** izleme ÇALIŞTI — 10:35–11:35 arası altı koşum "DIS YOL erisilemiyor (000)" yazdı, GitHub dış sondası kesinti kaydı (#1) açtı — ama görev 3. adımı yalnız RAPORLIYORDU. Kök neden: ağ kopup gelince funnel'ın ingress oturumu yenilenmedi; `funnel status` "Funnel on" derken üç ingress IP'si de TLS'de düşüyordu (2. adımın sorusu yetersiz). Onarım elle ölçüldü: `tailscale serve reset` + funnel'ı yeniden vermek 30 sn'de üç ingress'i 200'e döndürdü (yalnız funnel vermek yetmedi). Aynı el `saglik.ps1`'e kondu — yalnız uygulama yerelde sağlam ve adres çözülüyorken, iki deneme de düşünce; kayıt `onarim=1` (BUG #344); onarım sonrası yeniden ölçülür, 200 görmeden "onarıldı" denmez. Kapı `tests/test_tunel_onarimi_kapisi.py` (5 test, mutasyon 3/3). Kesinti süresi ~60 dk; kayıt #1 kapatıldı.
