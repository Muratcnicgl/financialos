"""
Net değer geçmişi (NetWorthSnapshot) backfill — HESAPLAMA KATMANI.

BUG #401 (BE-033): bu gövde `scripts/backfill_net_worth.py`de yaşıyordu ve `app/startup.py`
açılışta oradan içe aktarıyordu — uygulama çalışma zamanı `scripts/` paketine bağımlıydı
(katman ihlali; imaj kapısı bu yüzden bir betiği "zorunlu" sayıyordu). Mantık artık burada;
betik yalnız komut satırı sarmalayıcısı ve eski adları yeniden dışa aktarır.

Yöntem:
    - Bugün için generate_cockpit() (en doğru değer)
    - Geçmiş günler için: Account.balance'tan başla, target_date'ten sonraki
      transaction'ları ters uygula.
...
from app.models import (
    User, Account, AccountType, Transaction,
...
def _balance_at(db: Session, account: Account, target_date: date) -> float:
    ...
    # scope-exempt: hesap, çağıranın KENDİ kullanıcısının hesabıdır (snapshot_for kullanıcının
    # hesaplarını gezer); account_id sahipliği zaten daraltır. Kapsam kapısı taşıma sırasında
    # yakaladı — betikte görünmüyordu (BUG #401).
    after = db.query(Transaction).filter(  # scope-exempt: account_id sahipliği çağıranda
...
def snapshot_for(db: Session, user: User, target_date: date) -> dict:
    ...
    # BUG #402a (saat dilimi kapısı, taşıma sırasında yakalandı): betikte sunucu günü
    # kullanılıyordu; startup BUG #237 ile kullanıcının gününe göre aralık kuruyor, burası
    # "bugün mü?" kararını sunucu gününe göre veriyordu — yurt dışı kullanıcıda son gün
    # kokpit yerine geriye-doğru hesapla yazılabilirdi. Karar da kullanıcının günüyle.
    ...
    try:
        ...
        for user in users:
            ...
            while cur <= end_date:
                ...
                if verbose:
                    print(
                        # Para-birimi kapısı: sabit etiket yok (konsol satırı, sütun adı yeter)
                        f"  {cur}  Gorulen={snap['net_worth_seen']:>12,.2f}  "
                        f"Tam={snap['net_worth_full']:>12,.2f}  "
