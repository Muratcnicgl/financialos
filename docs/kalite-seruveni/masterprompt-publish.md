...
| H19 | **`alembic/env.py` config URL'ini yok sayıyor** — test/script içinden migration çağrısı GERÇEK DB'ye gidebilir (BUG #196). Canlı-veri migration provası buna bağlı | Claude (ölçüldü 5 Ağu) | P5.4 | ⬜ |
| H20 | Onboarding UI: demo veri + ilk-kurulum rehberi arayüze bağlanmalı (API hazır: `/api/onboarding/demo`) | Claude | P3.3 | ⬜ |
| H21 | Kullanıcı-tanımlı kural arayüzü: kural tiplerini UI'dan seçebilmeli (API hazır: `rule_type`/`rule_params`) | Claude | P3.5.2 | ⬜ |
...
| P5 | Dayanıklılık/gözlem | 🟡 | **Geri yükleme provası TAMAM (H14):** `scripts/restore.py` (onaysız yazmaz, bozuk yedeği reddeder, emniyet kopyası alır) + SQLite drill (7 test) + **PostgreSQL dump→drop→restore→doğrula** provası + runbook geri-yükleme bölümü. **Hata izleme TAMAM (#195):** kendi DB'mizde (dış servise veri gitmez), tekrarlar gruplanır, PII/sır maskelenir, izleme isteği düşürmez. **Kalan:** (a) **canlı-veri migration provası** — denendi, ERTELENDİ: `alembic/env.py` config'teki `sqlalchemy.url`'i yok sayıp `app.database` motorunu kullanıyor; test içinden `command.upgrade` çağrısı GERÇEK DB'ye gidiyor (bu turda canlı veri etkilenmedi, doğrulandı: 1 kullanıcı / 4 hesap / sürüm değişmedi). Önce env.py'nin URL'i config'ten alması düzeltilmeli (**BUG #196**), sonra prova alt-süreçte koşulmalı. (b) kapasite sınırları |
