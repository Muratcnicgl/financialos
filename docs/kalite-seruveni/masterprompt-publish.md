...
| H20 | Onboarding UI: demo veri + ilk-kurulum rehberi arayüze bağlanmalı | Claude | P3.3 | ✅ **BUG #262 ile GERÇEKTEN kapandı (7 Ağu).** Eski "✅ sıralı yol" iddiası yarımdı: kart ilk hesap eklenince kayboluyor, birincil düğmesi ölü bağlantıydı. Artık 4 adımlı ilerleme kartı — durum backend'de türetilir, her adım düğmesi gerçekten sekme açar, demo veri adım saymaz, gizleme geri alınabilir |
...
> R3 ile düzeltildi:** "provider kalite karşılaştırması yok" kısmen yanlıştı —
> `scripts/eval_runner.py` sağlayıcı başına koşuyor ve deterministik kriterlerle puanlıyor
> (eksik olan judge/yan-yana koşum/skor saklama). Ama o kriterlerden biri **kendi kopyasını**
> taşıyordu: `coach_eval._FAKE_DONE_RE` (5 kök), oysa ürün kodunda BUG #271'in ölçüp
> düzelttiği cevap zaten vardı. **Ölçüm (#271'in 12 cümlelik korpusu): eval 7'sini
> kaçırıyordu** → koç kalitesini koruyan araç, yeniden ortaya çıkan bir regresyonu YEŞİL
> puanlardı. Kopya **ters yönde de kırıktı**: nesneden bağımsız `tamamladı\w*` kökü
> "Analizi tamamladım" gibi meşru cümlelerde **4/4 yanlış-pozitif** verdi (sağlıklı sağlayıcı
> haksız düşerdi). **Fix:** tanıma tek kaynağa indi (`app.coach.sahte_tamamlama_iddiasi_var`);
...
>   diyordu — **iki belge çelişiyordu ve ölçünce ikisi de yarım çıktı** (L17'nin bir başka yüzü).
>   (a) Kart yalnız `accounts.length === 0` iken çiziliyordu → kullanıcı ilk hesabını ekler eklemez
>   kalan üç adım hiç yönlendirilmiyordu. (b) Kartın **birincil düğmesi ölüydü**: `<a href="#accounts">`,
>   oysa uygulama hash-router kullanmıyor. İkisi de sessiz (L28). Adım durumu artık `GET /api/onboarding/rehber`
...
> - **LİSANS/ATIF geçmişi:** `git-filter-repo` ile 574 commit işlendi, **311 mesajdan** Claude atıf satırları
...
> - **Taban:** 2150 passed / 18 skipped + 168 vitest + `npm run build` yeşil. Yeni migration `a3b4c5d6e7f8`.
>
...
> bir harcamanın **kredi kartına** yazılıp yazılmayacağı (`_CARD_CATEGORIES`, beş ad) ve hangi
> harcamanın "artış" uyarısına gireceği (`_PATTERN_EXCLUDED_CATEGORIES`). Kendi kategorisini
> adlandıran kullanıcıda iki kural da **sessizce** ölüyordu; ters yönde "market" deyip o
> harcamayı nakit yapan kullanıcının parası kart borcuna yazılıyor ve **iki bakiye birden**
> yanlış çıkıyordu. Sınıf taraması bugün canlı bir defekt buldu: hızlı girişin ürettiği
> `borc_geri_odeme` dışlama listesinde **hiç yoktu** — borç ödemeleri kişisel harcama
> sayılıyordu. Arayüzde ayrıca **üç ayrı, birbirinden farklı** sabit liste vardı ve kullanıcı
> setini hiçbir yerde kuramıyordu. Kategori artık bir KAYIT; karar ADDA değil BAYRAKTA
> (`app/category_rules.py` tek kaynak). `Transaction.category` bilinçli olarak serbest metin
> kaldı (FK, bilinmeyen kategoride kaydı reddetmeye zorlardı — sessiz kabulün ikizi sessiz RET).
> Silme = hedefe taşı (Actual Budget) ya da gizle (YNAB `hidden`); sistem kategorisi korumalı
> (YNAB `internal`). **Göç davranışı değiştirmez, sahipliği değiştirir.** 26 davranış testi +
> 249 kapı (backend + frontend aynası) + **mutasyon 6/6**. Projenin mevcut kapıları yeni
> tabloyu/uçları **kendiliğinden yakaladı** (KVKK sınıflandırması, izolasyon matrisi, boş-durum
> fixture'ı, temiz-DB şema kilidi) — dördü de kapatıldı.
> **Taban: 2451 passed / 18 skipped + 168 vitest + `npm run build` OK. Migration `b4c5d6e7f8a9`.**
>
...
| P3 | Operasyonel gerçeklik | ✅ | **Onboarding UI TAMAM (H20)** — Cockpit boş-durum kartı + demo veri akışı. **Kota TAMAM (ADR-041, BUG #188):** kullanıcı-başına LLM tavanı — paylaşılan sağlayıcı kotasını tek kişi tüketip diğerlerini kilitleyemez; tavan dolunca uygulama kapanmaz (Rules Engine deterministik). **#189** OAuth env adı kod↔doküman uyumsuzluğu, **#190** giriş yapmış kullanıcı şifre değiştiremiyordu. Sıfırdan-kullanıcı e2e ✅ (P3.5). **P3.2 (boş-veri hali her panelde) TAMAM (5 Ağu):** backend tarafı zaten kanıtlıydı ama kapsamı ölçülmemişti (**#217** — 87 uçtan 1'i taranıyordu); arayüz tarafı hiç sınanmamıştı → 13 panel × boş-durum + 13 panel × hata-durumu (54 test, gerçek boş-kullanıcı fixture'ı + sözleşme kayması kapısı). Bulunan gerçek defektler: **#218** (sonsuz istek döngüsü), **#219** (Bütçe paneli hata yolunda çöküyordu). **P3.3 (onboarding rehberi) TAMAM (7 Ağu, BUG #262):** buradaki "kalan" ile H20'nin "✅ sıralı yol" iddiası ÇELİŞİYORDU; ölçünce ikisi de yarım çıktı — kart ilk hesaptan sonra tümüyle kayboluyordu (kalan 3 adım hiç yönlendirilmiyordu) ve birincil düğme ölü `href="#accounts"` idi (uygulama hash-router kullanmıyor). Adım durumu artık backend'de tek kaynak (`GET /api/onboarding/rehber`), demo satırları adım saymaz, gizleme kalıcı + geri alınabilir. Ölü-CTA kapısı sekme listesini `App.jsx`'ten türetir (L27). **Opsiyonel demo veri zaten vardı** (BUG #194) — bu satır bayattı |
| P3.5 | **Ürünleşme (tek-kullanıcı DNA söküm)** | ✅ | **H1/H2/H3/H5/H21 ✅ + H4 saat dilimi ✅** (para birimi görüntüleme ADR-042 ile P8 öncesine planlı — yayın-engeli değil). ⚠️ **Düzeltme (BUG #237, 6 Ağu):** buradaki "saat dilimi ✅" iddiası da (ADR-042 ile birlikte) YARIM'dı — yalnız 7 router benimsemişti; koçun kaydettiği işlem SUNUCU gününe, yani farklı saat dilimindeki kullanıcı için KALICI olarak yanlış güne yazılıyordu. Kök sebep yapısaldı (handler'lara User geçmiyordu). Kapatıldı + **iddia artık statik kapıya bağlı** (`tests/test_saat_dilimi_kapisi.py` — her `date.today()` gerekçe zorunlu, 13 modül için adoption kilidi). Ders: L17. **H4'ün KATEGORİ ayağı da KAPANDI (7 Ağu, BUG #264 / ADR-046):** kategori seti artık
kullanıcının kayıtlarında; kart yönlendirmesi ve muhasebe-dışlaması sabit Türkçe adlara değil
`kart_varsayilani`/`sistem` bayraklarına bağlı. Kalan tek ayak **dil/i18n** (kapalı beta TR →
yayın-engeli değil). **H4'ün para birimi ayağı da KAPANDI (7 Ağu, BUG #256 / ADR-044):** biçimlendirme yedi ayrı yerde kodluydu (167 backend + 91 frontend ham "TL" sabiti); asıl bulgu `grounding` deseninin "TL" literaline gömülü olmasıydı → para birimi değişse doğrulama `{"ok": True}` ile sessiz-yeşile düşerdi, ayrıca koç bağlamının yatırım K/Z ve kart borcu satırları etiketsiz tutar yazıyordu. Görüntüleme tek kaynağa indi; **çok-para-birimli MUHASEBE bilinçli olarak kapsam dışı** (TRY kilidi ADR-044'te gerekçeli). Eski not: **H1/H2/H3/H5 ✅** (kullanıcı-tanımlı kural motoru #192, demo veri #194, iz temizliği). Eski not: **1. tur:** BUG #166 (metinlerde kişi adı → jenerik + statik kapı), #167 (TR normalize Kiril 'о' + sıra hatası → sessiz veri bozulması), #168 (banka markası koda gömülü → kullanıcının kendi hesap adları). **Kalan:** kullanıcı-tanımlı kural motoru (MC sabitleri), kişiselleştirme alanları, boş-durum + demo veri, sıfırdan-kullanıcı uçtan uca testi, yorum/docstring temizliği |
...
| P5 | Dayanıklılık/gözlem | ✅ | **Geri yükleme provası TAMAM (H14):** `scripts/restore.py` (onaysız yazmaz, bozuk yedeği reddeder, emniyet kopyası alır) + SQLite drill (7 test) + **PostgreSQL dump→drop→restore→doğrula** provası + runbook geri-yükleme bölümü. **Hata izleme TAMAM (#195):** kendi DB'mizde (dış servise veri gitmez), tekrarlar gruplanır, PII/sır maskelenir, izleme isteği düşürmez. **Canlı-veri migration provası TAMAM (#196):** `alembic/env.py` config URL'ini yok sayıyordu (test/script içinden migration GERÇEK DB'ye gidiyordu) → düzeltildi; prova artık izole DB'de koşuyor ve *gerçek DB'ye dokunulmadığı* da teste bağlı. **Fiyat sağlayıcı çöküşü TAMAM (H16 / #211):** fon-hisse zaten bayat işaretliydi; **döviz** kesintide TAMAMEN susuyordu → son bilinen kur `bayat`/`yas_dakika` ile sunuluyor, koç "şu anki" demiyor, 12 saatten eski değer hiç dönmüyor. **P5.5 (kapasite sınırları) TAMAM (7 Ağu, BUG #263):** kapasitenin yalnız DIŞARI bakan tarafı (Postgres `max_connections`) hesaplanmıştı; uygulamanın kendi eşzamanlılığı (anyio iş parçacığı havuzu 40) DB havuzunu (15) kat kat aşıyordu → havuz Postgres'e ulaşmadan uygulamanın İÇİNDE tükeniyor, `/api/ready` asılıyor, HEALTHCHECK konteyneri yeniden başlatıyordu (geçici yük dalgası → kalıcı kesinti). `app/capacity.py` tek kaynak (havuz hizalama + adlandırılmış yavaş-yol tavanları + açılışta fail-fast değişmezi). Sınıf taraması: premortem ucu koç tavanını atlıyordu, 8 LLM sağlayıcısından 7'si timeout'suzdu (SDK varsayılanı 600 sn), OAuth çağrılarında hiç timeout yoktu. Runbook'un küçük-VM tarifi değişmezi ihlal ediyordu → tarif düzeltildi ve artık belgeden okunup koşuluyor (22 test, mutasyon 3/3). **GERÇEK KALAN:** yok; P5 kapandı |
